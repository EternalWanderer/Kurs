...
public class MasterServerClient
{
	...
	public boolean trueFalseLogin;
	public boolean trueFalseRegistration;
	/**
	...
	public Room[] getGameRoomList()
	{
		...
		sendMessage(RETRIEVE_GAMELIST, null);
	...
	{
		sendMessage(REGISTER_GAMEROOM, new String[]{aGameName, aPort+""});
	}
	...
	{
		sendMessage(UNREGISTER_GAMEROOM, new String[]{aGameName, aPort+""});
	}
	
	
	public void registrationUser(String login, String password) {
		sendMessage(REGISTRATION, new String[] {login, password+""});
	}
	
	
	public void loginUser(String login, String password) {
		sendMessage(LOGIN, new String[] {login, password+""});
	}
	
	private void setTrueFalseLogin(String result) {
		if(result.equals("true")) {
			trueFalseLogin = true;
		}else {
			trueFalseLogin = false;
		}
		
	}
	
	private void setTrueFalseRegistation(String result) {
		if(result.equals("true")) {
			trueFalseRegistration = true;
		}else {
			trueFalseRegistration = false;
		}
	...
	public boolean getTrueFalseLogin() {
		return trueFalseLogin;
	}
	
	public boolean getTrueFalseRegistration() {
		return trueFalseRegistration;
	}
	 ...
	 */
	private static void sendMessage(String aCommand, String[] aData)
	{
		//Validate arguments
...
//					{
						if (command.equals(RETRIEVE_GAMELIST))
						{
							populateRoomList(data);
						}
						if(command.equals(LOGIN))
						{
							setTrueFalseLogin(data[0]);
						}
						if(command.equals(REGISTRATION)) {
							setTrueFalseRegistation(data[0]);
						}

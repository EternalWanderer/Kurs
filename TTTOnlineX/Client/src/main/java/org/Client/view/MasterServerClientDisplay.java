...
public class MasterServerClientDisplay extends JPanel implements ActionListener
{
	...
	private static MasterServerClient msClient;
	
	private static String nickname;
	
	
	public int getLenNickname() {
	...
	public static void setMSClient(MasterServerClient a) {
		msClient = a;
	}
	...
	private void joinSelectedGame()
	{
		...
		else if(validateMasterServerConnection())
		{
			...
			if (r.getHostPort() < 0) // Invalid game room
				return;
			GameClient gc = new GameClient(nickname);
...
/*!!!!!!!!!!!!!!!!!!!!!*/gc.sendID(); //////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
				...
				MainFrame.getInstance().setContentPane(clientDisplay);
				...
				else if (validateMasterServerConnection())
				{
					...
					do 
					{
						try
						{
							...
							final GameServer gameServer = new GameServer(ss, msClient, gameNameInput, nickname);
							final GameClient gameClient = new GameClient(nickname);

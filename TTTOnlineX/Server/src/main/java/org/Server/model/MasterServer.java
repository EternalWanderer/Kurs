...
public class MasterServer {
	...
	private  static ConcurrentHashMap<String, String> dataBaseUsers = null;
	private  static UserIO io = new UserIO("C:/Users/Akarumey/Desktop/University/5 Semester/TP/dataBase.txt");
	...
	public static boolean registrationUser(String login, String password) {
		initIO();
		if(!dataBaseUsers.containsKey(login)) {

			dataBaseUsers.put(login, password);

			io.write(dataBaseUsers);
	...
	public static boolean isValidCommand(String aCommand)
	{
		return (aCommand.equals(REGISTER_GAMEROOM) || 
				...
				aCommand.equals(LOGIN)             ||
				aCommand.equals(REGISTRATION)      ||
	...
	public class ClientHandler implements Runnable
	{
		...
		private void readMessage(String aInData)
		{
			...
			else if(command.equals(MasterServer.LOGIN))
			{
				String resultText;
				if(authorization(data[0], data[1])) {
					resultText = "true";
				}else {
					resultText = "false";
				}
				sendMessage(MasterServer.LOGIN, new String[] {resultText} );
			}
			else if(command.equals(MasterServer.REGISTRATION)) 
			{
				String resultText;
				if(registrationUser(data[0], data[1])) {
					resultText = "true";
				}else {
					resultText = "false";
				}
				sendMessage(MasterServer.REGISTRATION, new String[] {resultText} );
			}
		}
